...
import { mergePokedexEntries, type PokedexEntry } from "~/lib/pokemon/utils/mergePokedexEntries";
...
export function useRegionPokedexes(pokedexNames: string[] | null) {
  ...
  const queries = useQueries({
    queries: names.map((name) => ({
      ...
      queryFn: ({ signal }: { signal: AbortSignal }) => pokeApiClient.getPokedex(name, { signal }),
      enabled: names.length > 0,
    })),
  });

  const isLoading = names.length > 0 && queries.some((query) => query.isLoading);
  const isError = queries.some((query) => query.isError);
  const error = queries.find((query) => query.error)?.error ?? null;
  const allLoaded = names.length > 0 && queries.every((query) => query.data !== undefined);
